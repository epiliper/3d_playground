
#include "dummy.h"
#include "camera.h"
#include "glad.h"
#include "mouse.h"

// clang-format off
float CUBE_VERTICES[72] = {
  // front (z = 0.5)
  -0.5, -0.5,  0.5,  // 0
   0.5, -0.5,  0.5,  // 1
   0.5,  0.5,  0.5,  // 2
  -0.5,  0.5,  0.5,  // 3
  
  // back (z = -0.5)
  -0.5, -0.5, -0.5,  // 4
   0.5, -0.5, -0.5,  // 5
   0.5,  0.5, -0.5,  // 6
  -0.5,  0.5, -0.5,  // 7
  
  // left (x = -0.5)
  -0.5, -0.5, -0.5,  // 8
  -0.5, -0.5,  0.5,  // 9
  -0.5,  0.5,  0.5,  // 10
  -0.5,  0.5, -0.5,  // 11
  
  // right (x = 0.5)
   0.5, -0.5, -0.5,  // 12
   0.5, -0.5,  0.5,  // 13
   0.5,  0.5,  0.5,  // 14
   0.5,  0.5, -0.5,  // 15
  
  // top (y = 0.5)
  -0.5,  0.5,  0.5,  // 16
   0.5,  0.5,  0.5,  // 17
   0.5,  0.5, -0.5,  // 18
  -0.5,  0.5, -0.5,  // 19
  
  // bottom (y = -0.5)
  -0.5, -0.5,  0.5,  // 20
   0.5, -0.5,  0.5,  // 21
   0.5, -0.5, -0.5,  // 22
  -0.5, -0.5, -0.5,  // 23
};

unsigned int CUBE_INDICES[36] = {
  // front
  0, 1, 2,
  0, 2, 3,
  
  // back
  5, 4, 7,
  5, 7, 6,
  
  // left
  8, 9, 10,
  8, 10, 11,
  
  // right
  12, 13, 14,
  12, 14, 15,
  
  // top
  16, 17, 18,
  16, 18, 19,
  
  // bottom
  21, 20, 23,
  21, 23, 22,
};
// clang-format on
// float CUBE_VERTICES[72] = {
//   // front
//   -0.5, -0.5, 0.5,
//   0.5, -0.5, 0.5,
//   0.5, 0.5, 0.5,
//   -0.5, 0.5, 0.5,

//   // left
//   -0.5, -0.5, -0.5,
//   -0.5, -0.5, 0.5,
//   -0.5, 0.5, 0.5,
//   -0.5, -0.5, 0.5,

//   // right
//   0.5, -0.5, -0.5,
//   0.5, -0.5, 0.5,
//   0.5, 0.5, 0.5,
//   0.5, -0.5, 0.5,

//   // top
//   0.5, 0.5, -0.5,
//   -0.5, 0.5, -0.5,
//   -0.5, 0.5, 0.5,
//   -0.5, 0.5, -0.5,

//   // bottom
//   0.5, -0.5, -0.5,
//   -0.5, -0.5, -0.5,
//   -0.5, -0.5, 0.5,
//   -0.5, -0.5, -0.5,

//   // back
//   0.5, -0.5, -0.5,
//   -0.5, -0.5, -0.5,
//   -0.5, 0.5, -0.5,
//   0.5, 0.5, -0.5,
// };

// unsigned int CUBE_INDICES[48] = {
//   // front
//   0, 1, 2,
//   0, 4, 3,

//   // left
//   4, 5, 6,
//   4, 7, 5,

//   // right
//   8, 9, 10,
//   8, 11, 9,

//   // top
//   12, 13, 14,
//   12, 15, 13,

//   // bottom
//   16, 17, 18,
//   16, 19, 17,

//   // back
//   20, 21, 22,
//   20, 23, 21
// };
//clang-format on
//

Cube testCube = {
    .pos = {0, 0, 10},
    .color = {1, 0, 0, 1},
    .height = 2,
    .width = 1,
};

const char *cubeVert =
    "#version 330 core\n"
    "layout (location = 0) in vec3 pos;\n"
    "uniform mat4 view;\n"
    "uniform mat4 projection;\n"
    "uniform mat4 model;\n"
    "void main() {\n"
    "gl_Position = projection * view * model * vec4(pos.xyz, 1.0);\n"
    "}";

const char *cubeFrag = "#version 330 core\n"
                       "out vec4 out_color;\n"
                       "uniform vec4 color;\n"
                       "void main() {\n"
                       "out_color = color;\n"
                       "}";

void cubeRenderInit(Cube *c) {
  unsigned int vbo, vao, ebo, shader;

  glGenVertexArrays(1, &vao);
  glBindVertexArray(vao);

  glGenBuffers(1, &ebo);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(CUBE_INDICES), CUBE_INDICES,
               GL_STATIC_DRAW);

  glGenBuffers(1, &vbo);
  glBindBuffer(GL_ARRAY_BUFFER, vbo);
  glBufferData(GL_ARRAY_BUFFER, sizeof(CUBE_VERTICES), CUBE_VERTICES,
               GL_STATIC_DRAW);

  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void *)0);

  glEnableVertexAttribArray(0);

  shader = shaderFromCharVF(cubeVert, cubeFrag);

  RenderInfo ret = {.vao = vao, .vbo = vbo, .ebo = ebo, .shader = shader};

  c->rinfo = ret;
}

void cubeRender(Cube *c, RenderPayload r) {
  glBindVertexArray(c->rinfo.vao);
  glUseProgram(c->rinfo.shader);

  shaderSetMat4(c->rinfo.shader, "projection", *r.proj);
  shaderSetMat4(c->rinfo.shader, "view", *r.view);
  shaderSetVec3(c->rinfo.shader, "pos", c->pos);

  mat4 model;
  glm_mat4_identity(model);
  glm_translate(model, c->pos); // move to pos

  // apply length/width
  model[0][0] = c->width;
  model[1][1] = c->height;

  shaderSetMat4(c->rinfo.shader, "model", model);
  shaderSetVec4(c->rinfo.shader, "color", c->color);
  glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);
}

void testScenePrepare() { cubeRenderInit(&testCube); }

void testSceneRender() {
  fpsCameraPan(mouse.xpos, mouse.ypos, &fpsCamera);
  // glEnable(GL_DEPTH_TEST);

  glClearColor(0.2f, 0.3f, 0.3f, 1.0f);
  glClear(GL_COLOR_BUFFER_BIT);
  cubeRender(&testCube, (RenderPayload){.proj = &fpsCamera.projection,
                                        .view = &fpsCamera.view});
  // glDisable(GL_DEPTH_TEST);
}
